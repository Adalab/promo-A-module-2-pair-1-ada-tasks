...
const renderOneTask = (task) => {
    if (tasks.completed) {
      tasksList.innerHTML += `<li class= "tachado" >
             <input id="${tasks.id}" type= "checkbox" checked/>
             ${tasks.name}
             </li>`;
     } else {
       tasksList.innerHTML += `<li >
       <input id= "${tasks.id}" type= "checkbox" /> ${task.name}
      </li>`;
   }
 };
...
let tasks = [];
 ...
 .then((data) => {
  ...
  // const taskInput = document.querySelector(".js-text-task-add");
  // const taskFilter = document.querySelector(".js-text-task-filter");
  // const addTaskBtn = document.querySelector(".js-btn-add");
  // const filterTaskBtn = document.querySelector(".js-btn-filter");
